...
- Can open up to 3 trades simultaneously for the same symbol.
...
MAGIC = 445566
MAX_CONCURRENT_TRADES = 3 # MODIFIED: The maximum number of trades to open for a single symbol
    ...
    while bot_running and not _stop_event.is_set():
        try:
            ...
            if pending_signal:
                ...
                # --- MODIFIED: Check current position count against the max allowed ---
                ...
                if len(positions) >= MAX_CONCURRENT_TRADES:
                    ...
                    time.sleep(10) # Wait a bit longer before checking for new setups
                    continue
                # --- End Modification ---
                ...
                if pending_signal == "BUY" and tick.ask > setup_candle['High']:
                    ...
                    pending_signal = None; setup_candle = None
                    time.sleep(10) # Shorter pause between multiple trades
...
def run_strategy(symbol, timeframe, log_callback=None, profit_callback=None):
    ...
    log(f"🚀 EMA Breakout Strategy (Multi-Trade) Started on {symbol}", log_callback)
