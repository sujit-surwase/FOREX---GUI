...
def get_signal(df):
    ...
    # Momentum Candle Check: Body must be at least 60% of the total candle range
    candle_range = last.get('High', 0) - last.get('Low', 0)
    body_size = abs(last.get('Close', 0) - last.get('Open', 0))
    is_momentum_candle = body_size >= (candle_range * 0.6) if candle_range > 0 else False
        ...
        try:
            ...
            if signal != "WAIT":
                ...
                if not positions:
                    ...
                    # Pause for a bit after a trade to prevent instant re-entry
                    time.sleep(60) 
                else:
