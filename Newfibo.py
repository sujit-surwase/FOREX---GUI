...
def check_fibonacci_condition(df, fib_levels, direction):
    """
    FIXED: Check last 3 candles (including current closed one) for Fibonacci touch
    This ensures we catch the signal at the right time for live trading
    """
    if len(df) < 3:
        return False, None
    
    # Check last 3 candles to avoid missing signals due to timing
    for i in range(-3, 0):
        previous = df.iloc[i-1] if i > -3 else None
        current = df.iloc[i]
        
        current_close = current['close']
        current_open = current['open']
        prev_close = previous['close'] if previous is not None else current_open
        
        if direction == "uptrend":
            fib_618 = fib_levels['61.8']
            fib_382 = fib_levels['38.2']
            
            # Check if this candle touched 61.8%
            touched_618 = (current['low'] <= fib_618 <= current['high'])
            current_is_green = current_close > current_open
            current_moving_up = current_close > prev_close
            
            if touched_618 and current_is_green and current_moving_up:
                log(f"✅ Uptrend Fib Signal: Candle touched 61.8 ({fib_618:.5f}), bullish candle confirmed")
            ...
            # Check if this candle touched 38.2% (61.8% retracement)
            touched_382 = (current['low'] <= fib_382 <= current['high'])
            current_is_green = current_close > current_open
            current_moving_up = current_close > prev_close
            
            if touched_382 and current_is_green and current_moving_up:
                log(f"✅ Downtrend Reversal Fib Signal: Candle touched 38.2 ({fib_382:.5f}), bullish candle confirmed")
...
def get_signal(df):
    ...
    log(f"📊 Signal Check | RSI: {current_rsi:.2f} | SMA20: {current_sma_20:.5f} | SMA50: {current_sma_50:.5f} | Price: {current_price:.5f}")
    ...
    log("✅ FILTER 1: Swing points valid (older than 15 candles)")
    buy_signals += 1
    
    if ll_idx < hh_idx:
        direction = "uptrend"
        fib_levels = calculate_fibonacci_levels(hh, ll, direction)
        log("📈 CONDITION 2: Uptrend detected (LL → HH)")
    else:
        direction = "downtrend"
        fib_levels = calculate_fibonacci_levels(hh, ll, direction)
        log("📉 CONDITION 2: Downtrend reversal setup (HH → LL)")
    ...
    if fib_condition:
        log(f"✅ MANDATORY: Fibonacci signal confirmed. Proposed SL: {fib_sl:.5f}")
        stop_loss = fib_sl
    else:
        log("❌ MANDATORY: No Fibonacci signal. Waiting.")
    ...
    if buy_signals == 1: # This check relies on the "Swing points are valid" check above
        ...
        log(f"❌ FILTER 1: Swing points invalid.")
        
    if current_rsi < 50:
        log(f"✅ FILTER 2: RSI {current_rsi:.2f} < 50 - Valid")
        filter_signals += 1
    else:
        log(f"❌ FILTER 2: RSI {current_rsi:.2f} >= 50 - Invalid")
    
    if current_sma_20 > current_sma_50:
        log(f"✅ FILTER 3: SMA20 > SMA50 - High volatility confirmed")
        filter_signals += 1
    else:
        log(f"❌ FILTER 3: SMA20 <= SMA50 - Low volatility")
    ...
    if filter_signals >= 2:
        ...
        log(f"⚠️ WEAK SIGNAL: Mandatory Fib signal passed, but only {filter_signals}/3 filters met. Waiting.")
        return "WAIT", None
...
def trail_buy_stop_logic(ticket):
    """FIXED: Added bot_running checks with shorter sleep intervals to prevent deadlock"""
    ...
    while bot_running:
        ...
        if not current_price_tick:
            # FIXED: Shorter sleep with bot_running check
            for _ in range(5):
                if not bot_running:
                    return
                time.sleep(1)
        ...
        # FIXED: Sleep in 1-second intervals with bot_running checks
        for _ in range(10):
            if not bot_running:
                log(f"🛑 Trailer #{ticket} stopping due to bot_running=False")
                return
            time.sleep(1)
...
def strategy_loop():
    ...
    while bot_running:
        try:
            ...
            # Log when we check for signals (at candle close)
            last_closed_candle = df.index[-2] # -2 because -1 is current unclosed
            log(f"🕒 New candle closed at {last_closed_candle}. Checking for signals...")
            ...
            for ticket in list(active_trails.keys()):
                ...
                if not pos_exists:
                    log(f"🗑️ Position #{ticket} no longer open. Removing from trailer list.")
...
def run_strategy(symbol, timeframe, log_callback):
    ...
    log(" 3️⃣ Wait for price to hit 61.8% (uptrend) or 38.2% (downtrend reversal) [MANDATORY]")
    log(" 4️⃣ Confirm with bullish candle moving upward")
    log(" 5️⃣ RSI < 50")
    log(" 6️⃣ SMA20 > SMA50 (high volatility)")
    log(f" 7️⃣ Trailing stop: {TRAILING_PERCENT*100:.1f}% when {TRAILING_ACTIVATION*100:.1f}% of target reached")
    log(f" 8️⃣ Symbol: {SYMBOL}, Timeframe: {TIMEFRAME}, Max Volume: {VOLUME}")
    log("=" * 60)
...
def stop_strategy(log_callback):
    ...
    FIXED: Improved graceful shutdown with shorter timeout
    ...
    if strategy_thread and strategy_thread.is_alive():
        ...
        strategy_thread.join(timeout=5) # FIXED: Reduced timeout from 10s to 5s
        if strategy_thread.is_alive():
            log_callback("⚠️ Warning: Strategy thread did not terminate within 5 seconds.")
            log_callback("💡 Trailing stop threads will exit on their next check cycle.")
        else:
            log_callback("✅ Strategy thread terminated successfully.")
